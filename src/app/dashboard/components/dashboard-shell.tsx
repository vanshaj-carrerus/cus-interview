...
function DashboardShellContent({ children, guest = false }: Props) {
  ...
  return (
    <div className="flex min-h-screen bg-slate-50 text-secondary">
      <aside
      ...
      >
        <div className="fixed inset-x-0 top-0 z-30 flex h-14 items-center justify-between border-b border-slate-200 bg-white px-4 backdrop-blur-sm lg:hidden">
          <button
            type="button"
            aria-label="Open menu"
            onClick={() => setMobileOpen((v) => !v)}
            className="rounded-lg p-2 text-secondary transition hover:bg-slate-50"
          >
        ...
        >
          {guest ? <DashboardAccessRequired /> : children}
        </main>
      </div>
