...
export default async function CourseDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  await connectDB();
  const course = await LearningTrack.findOne({ slug, kind: "course" }).lean();
  if (!course) {
    return <p className="text-sm text-primary">Course not found.</p>;
  }

  const levels = await LearningLevel.find({ trackId: course._id }).sort({ levelNumber: 1 }).lean();
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold text-secondary">{String(course.title)}</h2>
        <p className="mt-1 text-sm text-secondary/70">{String(course.intro ?? "")}</p>
      </div>

      <div className="grid gap-4 lg:grid-cols-2">
        <form action={updateCourseAction} className="rounded-xl border border-primary/20 bg-white p-4">
          <p className="mb-3 text-sm font-semibold text-secondary">Edit Course</p>
          <input type="hidden" name="id" value={String(course._id)} />
        ...
        <form action={deleteCourseAction} className="rounded-xl border border-primary/20 bg-white p-4">
          <p className="mb-3 text-sm font-semibold text-secondary">Delete Course</p>
          <input type="hidden" name="id" value={String(course._id)} />
          <p className="mb-3 text-xs text-secondary/70">This removes the course with all levels and questions.</p>
      ...
      <form action={addLevelAction} className="rounded-xl border border-primary/20 bg-white p-4">
        ...
        <input type="hidden" name="trackId" value={String(course._id)} />
