...
async function importTrackFileAction(formData: FormData) {
  "use server";
...
export default async function LearningTracksPage() {
  await connectDB();
  const tracks = await LearningTrack.find({ kind: "track" }).sort({ title: 1 }).lean();
  const levels = await LearningLevel.find({ trackId: { $in: tracks.map((item) => item._id) } })
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <form action={importTrackFileAction} className="rounded-xl border border-primary/20 bg-white p-4">
          <p className="mb-3 text-sm font-semibold text-secondary">Import Track By File</p>
          <div className="space-y-2">
            ...
            <button className="rounded-lg bg-secondary px-3 py-2 text-sm font-medium text-white">Import Track</button>
          </div>
        </form>
      </div>
      ...
      <div className="overflow-x-auto rounded-xl border border-primary/20">
        <table className="min-w-full text-sm">
          ...
          <tbody className="divide-y divide-primary/10">
            {tracks.map((track) => (
              <tr key={String(track._id)}>
                <td className="px-4 py-3">{String(track.title)}</td>
                <td className="px-4 py-3 text-secondary/60">{String(track.slug)}</td>
                <td className="px-4 py-3">{levelCount.get(String(track._id)) ?? 0}</td>
                <td className="px-4 py-3">{questionCount.get(String(track._id)) ?? 0}</td>
                <td className="px-4 py-3">
                  <Link href={`/admin-panel/learning-tracks/${String(track.slug)}`} className="text-primary hover:underline">
                    Manage
                  </Link>
                </td>
              </tr>
