...
import { Code2, Globe, Server, FolderKanban } from "lucide-react";
...
export default function AdminProjectsPage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      router.refresh(); // Refresh in case there's a server component fetching projects
    } catch (err: any) {
  ...
  return (
    <div className="max-w-2xl bg-white p-6 rounded-xl shadow-sm border border-slate-200">
      ...
      {success && <div className="mb-4 p-3 bg-green-50 text-green-700 rounded-lg text-sm">Project successfully added!</div>}
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-1">Detailed Information (Markdown/HTML supported if you want)</label>
          <textarea name="details" rows={8} className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary focus:outline-none" placeholder="Enter detailed project requirements, setup instructions, etc."></textarea>
        </div>
